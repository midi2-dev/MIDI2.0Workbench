...
NAN_METHOD(get_UMP_Endpoints) {
   ...
   for(int i=0;i<numDevices;i++) {
    ...
    std::cout << "- : " << i <<" : " << a << '\n';
        ...
        SInt32 UMPGroupless;
            MIDIObjectGetIntegerProperty( lastEntityRef, kMIDIPropertyUMPCanTransmitGroupless, &UMPGroupless );
            std::cout << "listing MIDIDeviceRef : " << i <<" : kMIDIPropertyUMPCanTransmitGroupless " << UMPGroupless << '\n';
           ...
           // if(offline || strcmp(name,"MIDI 2.0")){ continue;} //Not a MIDI 2.0 Endpoint
            ...
            for (int b = 0; b < numEntities-1; b++) {
                v8::Local <v8::Object> blkO = Nan::New<v8::Object>();
                uint32_t EntityRef = MIDIDeviceGetEntity(MIDIDeviceRef, b);
...
NAN_METHOD(startListen) {
    ...
    if (info[0]->IsNumber() ){
      protocol = Nan::To<int>(info[0]).FromJust();
      if(protocol > 2) protocol=2;
